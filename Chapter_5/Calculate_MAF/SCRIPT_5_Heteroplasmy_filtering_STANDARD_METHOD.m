%% SCRIPT 5: Heteroplamy_filtering_STANDARD_METHOD.m
% Script for filtering heteroplasmies based on total read counts and frequencies.
% This script filters heteroplasmies according to STANDARD METHOD principles, and the equivalent 
% script for consensus method is SCRIPT 7
% Inputs for this script were generated by SCRIPT 4
%
% FILTER SETTINGS USED:
% Filter 1 - Allele supported by at least 1 read
% Filter 2 - Allele supported by at least 1 read in each sequencing direction
% Filter 3 - Appele supported by minimum 5 reads in each sequencing direction
% Filter 4 - Minimum 5 reads in each diection + overall allele frequency must be at least 0.1%
% Filter 5 - Minimum 5 reads in each diection + allele frequency must be at least 0.1% in each sequencing direction
% Filter 6 - Minimum 5 reads in each diection + overall allele frequency must be at least 1%
% Filter 7 - Minimum 5 reads in each diection + allele frequency must be at least 1% in each sequencing direction
%
% INPUTS from Output_4/Standard_method
%   > Samples.mat
%   > Positions.mat
%   > Sequences.mat
%   > Reads.mat
%   > Allele_count.mat      
%   > Allele_order.mat        - Index for allele sort order by total frequency - STANDARD METHOD
%   > Allele_bases.mat        - Sorted allele bases 
%   > Allele_reads.mat        - Sorted allele reads
%   > Allele_frequencies.mat  - Sorted allele frequencies
%   > Parameters     
%
% OUTPUTS 
% in Output_4/Standard_method
...
%   > Allele_reads        - filter passing allele reads
%   > Allele_frequencies  - filter passing allele frequencies
...
%   > Allele_reads        - non-filtered allele reads
%   > Allele_frequencies  - non-filtered allele frequencies
...
paths.output = fullfile(paths.root,'Output_5');
%paths.output = fullfile(paths.root,'Output_5/Standard_method');

% Set input paths
in_paths = struct();
in_dir = 'Output_4/Standard_method';

% from Output_4/Standard_method
...
in_paths.Reads              = fullfile(paths.root,in_dir,'Reads.mat');
in_paths.Allele_count       = fullfile(paths.root,in_dir,'Allele_count.mat');
%in_paths.Allele_order       = fullfile(paths.root,in_dir,'Allele_order.mat');
in_paths.Allele_bases       = fullfile(paths.root,in_dir,'Allele_bases.mat');
in_paths.Allele_reads       = fullfile(paths.root,in_dir,'Allele_reads.mat');
in_paths.Allele_frequencies = fullfile(paths.root,in_dir,'Allele_frequencies.mat');
...
%data.Allele_order       = load(in_paths.Allele_order, 'A','C','G','T');
data.Allele_bases       = load(in_paths.Allele_bases, 'Major','Minor_1','Minor_2','Minor_3');
data.Allele_reads       = load(in_paths.Allele_reads, 'Major','Minor_1','Minor_2','Minor_3');
data.Allele_frequencies = load(in_paths.Allele_frequencies, 'Major','Minor_1','Minor_2','Minor_3');
...
temp_idx_low_depth = (...
    data.Reads.For < parameters.min_read_depth | ...
    data.Reads.Rev < parameters.min_read_depth );
...
temp_filter_settings.Reads_overall  =[ 1;   2;   10;    10;    10;   10;    10];
temp_filter_settings.Reads_each_dir =[ 0;   1;    5;     5;     5;    5;     5];
...
for temp_filter_num = 1:numel(temp_filters.Filter)
    ...
    temp_min_reads_total   = temp_filters.Reads_overall(temp_filter_num);
    temp_min_reads_per_dir = temp_filters.Reads_each_dir(temp_filter_num);
    ...
    for temp_allele = temp_alleles
        ...
        temp_reads  = data.Allele_reads.(temp_allele);
        temp_allele_freqs = data.Allele_frequencies.(temp_allele);
    
        % Find indices of alleles passing each filter
        temp_mask = ... % 1 = allele passes filter, 0 = Allele fails filter
            temp_reads.All >= temp_min_reads_total   & ... % Apply min reads overall
            temp_reads.For >= temp_min_reads_per_dir & ... % Apply min reads in F dir
            temp_reads.Rev >= temp_min_reads_per_dir & ... % Apply min reads in R dir
...
for temp_allele = ["Major", "Minor_1", "Minor_2", "Minor_3"]
    ...
    for temp_dir = ["All", "For", "Rev"]
        data.Allele_reads.(temp_allele).(temp_dir)(~temp_mask_pass) = NaN;
        data.Allele_frequencies.(temp_allele).(temp_dir)(~temp_mask_pass) = NaN;
    end
end

% Filter total reads
for temp_dir = ["All", "For", "Rev"]
    data.Reads.(temp_allele).(temp_dir)(~temp_mask_pass) = NaN;
end
...
for temp_filter = data.Heteroplasmy_filters.Filter'
    ...
    for temp_allele = ["Major", "Minor_1", "Minor_2", "Minor_3"]
        ...
        for temp_dir = ["All", "For", "Rev"]
            ...
            temp_reads = data.Allele_reads.(temp_allele).(temp_dir);
            temp_reads(~temp_allele_mask) = NaN;
        
            % Get filter-passing allele frequencies
            temp_frequency = data.Allele_frequencies.(temp_allele).(temp_dir);
            temp_frequency(~temp_allele_mask) = NaN;

            % Save filtered reads & frequencies into appropriate FILTER struct
            data.(temp_filter).Allele_reads.(temp_allele).(temp_dir)       = temp_reads;
            data.(temp_filter).Allele_frequencies.(temp_allele).(temp_dir) = temp_frequency;
        end
    end
...
for temp_filter = data.Heteroplasmy_filters.Filter'
    for temp_dir = ["All", "For", "Rev"]
        for temp_var = ["Allele_reads","Allele_frequencies"]
...
data.NoFilter.Allele_reads = data.Allele_reads;
data.NoFilter.Allele_frequencies = data.Allele_frequencies;
